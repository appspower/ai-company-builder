...
function MissionsPage() {
  ...
  if (filterPriority !== 'all') filteredMissions = filteredMissions.filter((m) => m.priority === filterPriority)
  if (searchQuery.trim()) {
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 p-6 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <Select value={filterDept} onValueChange={(v) => setFilterDept(v ?? 'all')}>
            <SelectTrigger className="w-36 h-9"><SelectValue placeholder="부서" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={filterPriority} onValueChange={(v) => setFilterPriority(v ?? 'all')}>
            <SelectTrigger className="w-32 h-9"><SelectValue placeholder="우선순위" /></SelectTrigger>
            <SelectContent>
          ...
          <div className="flex border rounded-lg overflow-hidden ml-auto">
            <button onClick={() => setViewMode('board')} className={`p-2 transition-colors ${viewMode === 'board' ? 'bg-muted' : 'hover:bg-muted/50'}`} title="보드 뷰">
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button onClick={() => setViewMode('list')} className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-muted' : 'hover:bg-muted/50'}`} title="리스트 뷰">
        ...
        {filteredMissions.length > 0 ? (
          <div className="flex-1 overflow-auto">
            {viewMode === 'board' ? (
              <KanbanBoard missions={filteredMissions} onMissionClick={(id) => selectMission(id)} onMissionMove={moveMission} />
            ) : (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <Button onClick={() => setCreateOpen(true)}>{isFiltered ? '새 미션 만들기' : '첫 미션 만들기'}</Button>
          </div>
        )}
      </div>

      {/* 인라인 상세 패널 (읽기 — 딤 없이 나란히) */}
      {selectedMission && (
        <MissionInlineDetail
      ...
      {/* 결재 모달 (승인/반려 시에만 Sheet) */}
      <MissionDetailPanel
        mission={approvalMission}
        open={!!approvalMissionId}
        onClose={() => setApprovalMissionId(null)}
